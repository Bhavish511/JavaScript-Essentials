...
function findspecialization() {
    const foundemp = employees.filter(employee => employee.specialization === 'JavaScript');
    const specialEmployeesDisplay = foundemp.map((employee, index) => `<p>${employee.id}: ${employee.name} - ${employee.department} - $${employee.salary} - ${employee.specialization}</p>`).join('');
